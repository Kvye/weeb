...
def test_anime_search():
    """trying to test if we get the same results"""
    f = open("query.txt", "r")
    check = f.read()
    f.close()
...
def test_is_sortedlist_empty(): 
    """trying to test if the sortedlist is empty"""
    emptylist = [] # our expected result

    assert query.sortedlist == emptylist

def test_specify():
    """trying to test if specify is null"""
    assert query.specify == "null"


def test_get_anime(): 
    """trying to test if get_anime returns the anime we search for"""
...
def test_sort(): 
    """test that sort returns the sorted result we search for"""
